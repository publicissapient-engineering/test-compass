...
import static com.publicissapient.anaroc.util.SeleniumConstants.SCROLL_PAGE_UP;

public class ScrollUp extends SeleniumActionWindow {
    public ScrollUp(StepDefinition stepDefinition, WebDriver webDriver, Map<String, String> xPaths) {
    ...
    protected Result executeWithSelenium() {
        ...
        if(SeleniumHelper.isNumeric(getXpath())) {
            pixel = Integer.valueOf(getXpath()) * -1;
        }else {
            return getFailedResult(SeleniumHelper.formatString(NOT_A_NUMBER_ERR_MSG.value(), "pixel"));
        }
        String js = SeleniumHelper.formatString(SCROLL_PAGE_UP.value(), String.valueOf(pixel));
